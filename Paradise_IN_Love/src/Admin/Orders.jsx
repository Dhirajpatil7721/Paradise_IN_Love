...
//       try {
//         setLoading(true);
//         const res = await axios.get('http://localhost:8080/api/order/all-order-list', {
...
//                         <h3 className="text-base sm:text-lg font-bold text-rose-800 mr-2">Order #{order.orderId}</h3>
//                         <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(order.size)} mb-1 sm:mb-0`}>
//                           {order.size}
//                         </span>
//                         <span className={`ml-2 px-2 py-1 rounded-full text-xs font-medium ${getPaymentColor(order.userId.name)} mb-1 sm:mb-0`}>
//                           {order.userId.name}
//                         </span>
//                       </div>
...
//                         <p className="text-lg font-bold">₹{order.totalAmt.toFixed(2)}</p>
...
import { useState, useEffect } from 'react';
import { Link, useLocation } from 'react-router-dom';
import { FiChevronDown, FiChevronUp, FiHome, FiShoppingBag, FiPlus, FiUsers, FiSettings, FiMenu, FiX, FiSearch, FiCalendar, FiFilter, FiDollarSign, FiUser, FiShoppingCart, FiLogOut, FiMapPin, FiPackage, FiCreditCard } from 'react-icons/fi';
import axios from 'axios';
        ...
        <div className="p-4 sm:p-5 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center">
          <div className="mb-3 md:mb-0">
            <div className="flex items-center flex-wrap">
              ...
              <h3 className="text-base sm:text-lg font-bold text-rose-800 mr-2">{order.product_details.name}</h3>
              <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(order.product_details.size)} mb-1 sm:mb-0`}>
                {order.product_details.size}
              </span>
              <span className={`ml-2 px-2 py-1 rounded-full text-xs font-medium ${getPaymentColor(order.quantity)} mb-1 sm:mb-0`}>
                {order.quantity}
              </span>
            </div>
            <p className="text-sm text-gray-600 mt-1">
                ...
                <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <div className="flex items-center">
                          <div className="flex-shrink-0 h-10 w-10 bg-gray-200 rounded-md overflow-hidden">
                            {order.product_details.image && order.product_details.image.length > 0 ? (
                              <img className="h-full w-full object-cover" src={order.product_details.image[0]} alt={order.product_details.name} />
                            ) : (
                  ...
                  <tfoot>
                    <tr>
                      ...
                      <td className="px-6 py-3 text-sm font-bold">₹{order.subTotalAmt.toFixed(2)}</td>
                    </tr>
                    <tr>
                      <td colSpan="4" className="px-6 py-3 text-right text-sm font-medium text-gray-500">Total</td>
                      <td className="px-6 py-3 text-lg font-bold text-rose-600">₹{order.subTotalAmt.toFixed(2)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
