...
function UpdateProduct_Popup() {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            const response = await fetch(`${API_URL}/product/update-product-details`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                credentials: "include",
                body: JSON.stringify({
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="pt-4">
                            <motion.button
                                ...
                                whileTap={{ scale: 0.98 }}
                                disabled={isLoading}
                                className={`w-full bg-gradient-to-r from-pink-600 to-purple-600 text-white py-3 rounded-lg font-medium hover:from-pink-700 hover:to-purple-700 transition-all duration-300 shadow-md ${isLoading ? 'opacity-70 cursor-not-allowed' : ''
                                    }`}
                            >
